      ...
      <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar scroll-smooth">
        <AnimatePresence>
          {items.map((item) => {
            ...
            const remainingSecs = item.duration - item.currentTime;
            ...
            return (
              <motion.div
                ...
                className="flex-none w-[240px] sm:w-[280px] md:w-[320px] aspect-video bg-surface-dark/60 rounded-xl overflow-hidden cursor-pointer relative group border border-white/5 shadow-lg flex flex-col justify-between"
              >
                ...
                <div className="relative z-10 flex items-center justify-between p-3">
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-full bg-black/60 backdrop-blur-md text-gray-400 hover:text-white hover:bg-brand/80 transition-all duration-200 border border-white/5 opacity-0 group-hover:opacity-100"
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                  <div className="w-12 h-12 rounded-full bg-brand flex items-center justify-center shadow-lg transform scale-90 group-hover:scale-100 transition-transform duration-300">
                    <Play size={20} fill="currentColor" className="text-white ml-0.5" />
                  </div>
                </div>

                {/* Bottom Bar: Title, progress and remaining time */}
                <div className="relative z-10 p-3 mt-auto flex flex-col justify-end">
                  <h3 className="font-bold text-white text-xs sm:text-sm line-clamp-1 drop-shadow-md">
                    {item.title}
                  </h3>
                  
                  {item.type === 'tv' && item.season && item.episode && (
                    <p className="text-[10px] text-brand font-medium drop-shadow mt-0.5">
